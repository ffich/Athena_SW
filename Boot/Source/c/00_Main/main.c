...
int main(void)
{
   ...
   Lcd_WriteConstString("BOOTLOADER MODE");
   /* Move to line 2 */
   Lcd_WriteCommand(CURSOR_LINE_2);
   Lcd_WriteConstString("Checking USB...");
   /* Set backlight on */
...
{	
   /* Function Pointer */
   void (*AppPointer)(void);
   
   /* Point to application & call */
   AppPointer = (void (*)(void))USER_APP_RESET_ADDRESS;
   AppPointer();
}
		...
		switch(HexRecordSt.RecType)
		{
			...
			case END_OF_FILE_RECORD:  //Record Type 01, defines the end of file record.
				...
				HexRecordSt.ExtLinAddress.Val = 0;
                                 ...
                                 /* Move to line 2 */
                                 Lcd_WriteCommand(CURSOR_LINE_2);
                                 Lcd_WriteConstString("Flashing Done!");  
...
* Output:       BOOL
* Author:	F.Ficili
* Description:	Check if a valid application is present.
* Date:		04/11/15
************************************************************************/
BOOL ValidAppPresent (void)
{
   /* Pointer to app */
   UINT32 *AppPtr;
   volatile UINT8 ReturnValue;
   
   /* Point to application reset vector */
   AppPtr = (UINT32 *)USER_APP_RESET_ADDRESS;   
   /* Check if the pointer is pointing to a valid application */
   if((*AppPtr) == 0xFFFFFFFF)
   {
      ReturnValue = FALSE;
   }
